...
+
コマンドレットで使用できるパラメータとその説明については、 Get-Help_command_name _ を実行して取得できます。または、 https://docs.netapp.com/us-en/snapcenter-cmdlets-49/index.html["コマンドレットリファレンスガイド"^]
...
** Linux：_/var/opt/scc/allowed_commands.config SnapCenter
...
.. スケジュールを設定するポリシーの[Configure Schedules]列でを選択しますimage:../media/add_policy_from_resourcegroup.gif["リソースグループからポリシーを追加"]。
